...
### .命令 重复上一次修改
### !G 命令
    普通不是下输入，vim会在命令行显示.,$! 含义为范围从光标当行开始到最后一行结束，然后把当前行到最后一行作为输入传递给叹号后面的程序
...
### ga命令 
    显示当前字符十进制、十六进制和八进制编码
### ctrl-o命令
    跳转到跳转列表中第 [count] 个更早的光标位置（这不是一个移动命令）,与 ctrl-i 方向相反，后者向前移动（向更新的时间点）
## 操作符+动作命令=操作
### d{motion}命令 
...
### 常用命令
    ...
    9. :命令 可是模式下在命令行输入冒号，则命令行会显示'<,'>表示选中的开始行和技术行
# 命令行模式
    vim回记录在命令行模式下的命令历史。先按:键切换到命令行模式，在保持提示符为空的情况下按up键，或者down键，之前执行过的命令就回填充到命令行上。
    先按:c键，再按up或者down键，此时只有c打头的执行过的命令回被填充到命令行
    可以使用set history=200 修改vim历史命令的记录数，默认是20，也可以将set history=200放在vimrc文件中。
### 常用命令
#### :print
...
#### :copy命令
    1. 简写为co或者t
    2. 格式 :[range]co[py] {address} [range]如果省略默认为当前行,例如 :6t. 将第6行复制到当前行下方、:6t0 将第6行、'<,'>t0 将高亮选中的区域复制到文档初始位置
#### :move命令
    1. 简写为m
    2. 格式 :[range]m[ove] {address}
#### :[range]norm[al] {commands} 
    1. 允许你在命令行模式中执行普通模式的命令，实现了两种模式间的桥梁。
    2. 在指定的 {range} 范围内的每一行上执行普通模式命令 {commands}。
    3. 在执行 {commands} 之前，光标会定位到该行的第一列。其他方面与不带范围的 :normal 命令相同。
#### ctrl-r{register}命令
    将指定寄存器的内容粘贴到当前位置
#### ctrl-r=5*8命令
...
#### 运行shell命令
    vim命令行模式中，在命令前加叹号(!)前缀可以调用外部程序。例如:!ls 在当前目录下执行操作系统的ls命令，:ls 执行vim内容的ls命令，查看缓冲区文件列表
    如果不想关闭当前文件，但是又需要执行多条shell命令，可以使用:shell启动一个交互的shell会话，使用exit命令退出此shell并返回vim
#### read !shell命令
    将shell命令的执行结果读入到当前打开的文件
#### write !shell命令
    将当前打开文件的内容做为shell命令的指定输入
#### 使用外部命令过滤缓冲区内容
    当给定一个范围是，:!{cmd} 命令就有了不同的含义。有[range]指定的行会传给{cmd}作为标准输入，然后用{cmd}的输出覆盖[range]内原本的内容，换一种说法就是[range]内的文本会被指定的{cmd}过滤。例如：cmdline_mode/emails.csv    .,$!sort -t',' -k2   对光标所在行到文件最后一行，根据第二列排序，然后替换原有内容
